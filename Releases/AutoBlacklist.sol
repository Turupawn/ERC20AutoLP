...
contract MyToken is ERC20, Ownable
{
    ...
    function swap() private lockTheSwap {
        ...
        uint256 totalFee = _buyFeeCollected + _sellFeeCollected;
        ...
        // Send to marketing
        uint256 amountBuy = (amountFee * _buyFeeCollected) / totalFee;
        if(amountBuy > 0) sendViaCall(payable(buyWallet), amountBuy);

        // Send to team
        uint256 amountSell = (amountFee * _sellFeeCollected) / totalFee;
        if(amountSell > 0) sendViaCall(payable(sellWallet), amountSell);
        
        _buyFeeCollected = 0;
        _sellFeeCollected = 0;

        emit Swap(totalFee, amountBuy, amountSell);
    }
    ...
    function collectFee(
        ...
        uint256 feesCollected = calculateFee(p2p ? 2 : sell ? 1 : 0, amount);
        if(feesCollected != 0) 
        {
    ...
    ) internal virtual override(ERC20)
    {
        ...
        if (isFeeActive && !isTaxless[from] && !isTaxless[to] && !inSwap) {
            collectFee(from, amount, to == pair, from != pair && to != pair);
        }
    }
